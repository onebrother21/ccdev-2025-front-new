...
import { AuthUser,AuthToken,AuthRoles } from '../models';
...
export class AuthController extends InMemoryApiController<AuthUser,"users"> {
  ...
  generateTkn(m:AuthUser,type:"auth"|"req",role?:AuthRoles){
  ...
  register = (req:HttpRequest<any>) => {
    try {
      ...
      data.agree?m.agree = new Date():null;
      m.meta = {...m.meta,registered:new Date()};
      m.token = this.generateTkn(m,"req","USER");
      this._save(m,i);
      this.jobQ.addJob("notification",{
        type:"registration",
  ...
  override update = (req:HttpRequest<any>) => {
    try {
      ...
      m.token = this.generateTkn(m,"auth");
      m.status = {name:"active",time:new Date()};
      m.meta = {...m.meta,updated:new Date()};
      this._save(m,i);
      return this.ok(m.jsonAuth(true));
    }
    catch(e:any){
