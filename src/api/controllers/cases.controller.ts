...
export class CasesController  extends InMemoryApiController<Case,"cases"> {
  ...
  search = (req:HttpRequest<any>,seperator?:string) => {
    ...
    const m = this._searchInArray(this._load() as Case[],q,fields);
  ...
  saveActivity = (req:HttpRequest<any>) => {
    ...
    const m = this._findone({slug:this.paramFromUrl(req.url)}).m;
    /*
    const n:Case["activity"][0] = req.body.data;
    const a:Case["activityLog"][0] = {
      user:u.username,
      action:"added",
      type:n.type,
      time:new Date(),
